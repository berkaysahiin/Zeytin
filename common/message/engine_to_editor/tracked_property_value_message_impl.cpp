...
module zeytin.common.message.engine_to_editor.tracked_property_value;

std::string TrackedPropertyValueMessage::get_type() const {
    return "tracked_property_value";
}

std::string TrackedPropertyValueMessage::as_json() const {
...
bool TrackedPropertyValueMessage::from_json(const rapidjson::Value& message) {
    if (!message.IsObject()) {
        return false;
    }
