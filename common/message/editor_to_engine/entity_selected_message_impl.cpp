...
module zeytin.common.message.editor_to_engine.entity_selected;

std::string EditorEntitySelectedMessage::get_type() const {
    return "entity_selected";
}

std::string EditorEntitySelectedMessage::as_json() const {
...
bool EditorEntitySelectedMessage::from_json(const rapidjson::Value& message) {
    if (!message.IsObject()) {
        return false;
    }
