...
module zeytin.common.message.editor_to_engine.tracked_property_request;

std::string EditorTrackedPropertyRequestMessage::get_type() const {
    return "tracked_property_request";
}

std::string EditorTrackedPropertyRequestMessage::as_json() const {
...
bool EditorTrackedPropertyRequestMessage::from_json(const rapidjson::Value& message) {
    if (!message.IsObject()) {
        return false;
    }
