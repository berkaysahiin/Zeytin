...
module zeytin.component.list;
import zeytin.filewatcher;
import zeytin.resource;

ComponentList::ComponentList() : m_variant_watcher(ResourceManager::get().get_components_paths(), std::chrono::milliseconds(500)) {
    load_variants();
    start_watching();
}

void ComponentList::load_variants() {
    m_variants.clear();
    ...
    for(const auto& entry : std::filesystem::directory_iterator(ResourceManager::get().get_components_paths())) {
        ...
        m_variants.emplace_back<ComponentDocument>(std::move(name));
    }
...
void ComponentList::load_variant(const std::filesystem::path& path) {
    ...
    if (it != m_variants.end()) {
        ...
        auto& rv = m_variants.emplace_back(ComponentDocument(std::move(name)));
        rv.load_from_file();
    }
}

void ComponentList::start_watching() {
